...
<div class="container">
    ...
    <div class="row" style="margin-bottom: 3em">
        <div class="col-md-4 product-image">
            <div>
                <img src="{{ asset('storage/'.$product->image) }}" width="100%" height="100%" id="current-image">
            </div>
            <div class="image-thumbnails">
                @if ($images)
                    <img src="{{ asset('storage/'.$product->image) }}" class="image-thumbnail active">
                    @foreach ($images as $image)
                        <div>
                            <img src="{{ asset('storage/'.$image) }}" class="image-thumbnail">
                        </div>
                    @endforeach
                @endif
            </div>
        ...
        <div class="product-details col-md-5 offset-md-1">
            ...
            @if ($product->quantity > 0)
                <form action="{{ route('cart.store') }}" method="POST">
                    ...
                    <button type="submit" class="btn custom-border-n">Ajouter au panier</button>
                </form>
            @endif
        </div>
    </div>
